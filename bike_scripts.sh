...
my_input = open('example_data.csv')
for f in my_input:
	...
	if re.search("starttime", f): 
		...
		if bike_ID == new_bike_ID:
			...
			if end_ID != start_ID:
				...
				nbike_ID = f.split(",")[0]
				nend_time = f.split(",")[4]
				ne_time = end_time.replace("2013-","")
				nend_t = e_time.replace("-",",")
				ne_t = end_t.replace(":",",")
				nend = e_t.replace(" ",",")
				nending_time = end.split(",")
				if nending_time[0] == '09':
					nmonth_in_s = int(First_8_months)
					nday_in_s = int(ending_time[1]) * Day_count
					nhour_in_s = int(ending_time[2]) * Hour_count
					nmin_in_s = int(ending_time[3]) * Min_count
					nsec_in_s = int(ending_time[4])
				elif nending_time[0] == '10':
					nmonth_in_s = Sep
					nday_in_s = int(ending_time[1]) * Day_count
					nhour_in_s = int(ending_time[2]) * Hour_count
					nmin_in_s = int(ending_time[3]) * Min_count
					nsec_in_s = int(ending_time[4])
				elif nending_time[0] == '11':
					nmonth_in_s = Oct
					nday_in_s = int(ending_time[1]) * Day_count
					nhour_in_s = int(ending_time[2]) * Hour_count
					nmin_in_s = int(ending_time[3]) * Min_count
					nsec_in_s = int(ending_time[4])
				elif nending_time[0] == '12':
					nmonth_in_s = Nov
					nday_in_s = int(ending_time[1]) * Day_count
					nhour_in_s = int(ending_time[2]) * Hour_count
					nmin_in_s = int(ending_time[3]) * Min_count
					nsec_in_s = int(ending_time[4])
				nTotal_end_time = nmonth_in_s + nday_in_s + nhour_in_s + nmin_in_s + nsec_in_s
				time_at_base = Total_start_time - nTotal_end_time
				baseTime.append(time_at_base)
		end_ID = f.split(",")[1]
		bike_ID = f.split(",")[0]
		end_time = f.split(",")[4]
		e_time = end_time.replace("2013-","")
...
print numpy.mean(baseTime)
# No. of characters 9087